...
import type { DadosVeiculo, DadosIpva, EstadoIpva } from './types.js';
...
export async function consultarPlaca(placa: string): Promise<{
  ...
  const historico: DadosIpva['historico'] = [];
  ...
  const ipva: DadosIpva = { valorVenal, aliquota, valorIpva, historico };

  // Tabela 4: IPVA por estado (Estado | Valor Venal | Taxa | Valor IPVA)
  const estados: EstadoIpva[] = [];
